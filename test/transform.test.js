...
test('skips non-jsx and node_modules ids', () => {
  assert.equal(transform('<Button>Hi</Button>', '/project/src/util.ts'), undefined)
  assert.equal(transform('<Button>Hi</Button>', '/project/node_modules/x/App.tsx'), undefined)
})

test('exclude option skips matching ids (string and RegExp)', () => {
  const plugin = clickToSource({ exclude: ['generated/', /\.stories\.tsx$/] })
  plugin.configResolved({ root: '/project' })
  assert.equal(plugin.transform('return <A>x</A>', '/project/src/generated/App.tsx'), undefined)
  assert.equal(plugin.transform('return <A>x</A>', '/project/src/Button.stories.tsx'), undefined)
  assert.match(plugin.transform('return <A>x</A>', '/project/src/App.tsx').code, /data-cts-loc/)
})
